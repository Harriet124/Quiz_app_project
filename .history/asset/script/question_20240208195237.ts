...
function markOption(optionsList: HTMLUListElement, optionElement: HTMLElement) {
  ...
  // Check if there is a stored option in localStorage
  const storedOptionIndex = localStorage.getItem("markedOptionIndex");

  if (storedOptionIndex && !submitted) {
    
  }

  if (!submitted) {

    const optionsList = optionsList.querySelectorAll("li");

    options.forEach((option, index) => {
      if (index.toString() === storedOptionIndex) {
        option.classList.add("marked");
        option.style.outline = "2px solid rgba(167, 41, 245, 1)";
    ...
    // Store the selected option index in localStorage
    const options = optionsList.querySelectorAll("li");
    options.forEach((option, index) => {
      if (option === optionElement) {
        localStorage.setItem("markedOptionIndex", index.toString());
      }
    });
  }
}
...
function toggleDarkModeStyles(mode: string) {
  ...
  // Update styles for the score board (add the following lines)
  // const scoreBoard = document.querySelector(".score-flex") as HTMLElement;
  // scoreBoard.style.backgroundColor = mode === "#f4f6fA" ? "#ffffff" : "#3B4D66";
  // scoreBoard.style.color = mode === "#f4f6fA" ? "#313e51" : "#ffffff";
