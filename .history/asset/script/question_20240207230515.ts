...
questionListContainer.innerHTML = "";
const existingFlexContainer = document.querySelector(".container");
...
// Modify the function to accept an additional parameter for the score page elements
function toggleDarkModeStyles(mode, scorePageElements) {
  const slider = document.querySelector(".slider") as HTMLElement;
  const body = document.body;
  ...
  // Apply mode to score page elements
  if (scorePageElements) {
    scorePageElements.forEach((element) => {
      element.style.color = mode === "#f4f6fA" ? "#ffffff" : "#313e51";
      element.style.backgroundColor = mode === "#f4f6fA" ? "#3B4D66" : "#F4F6FA";
    });
  }
}

// Call the function with the score page elements when toggling the mode
toggleDarkModeStyles(mode, [scorePageElement1, scorePageElement2, ...]);
