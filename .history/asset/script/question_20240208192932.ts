...
let isOptionSelected = false; // Add this variable
...
optionsList.querySelectorAll("li").forEach((option) => {
  ...
  option.addEventListener("click", function optionClickHandler() {
    if (!isOptionSelected) {
      markOption(optionsList, this);
      isOptionSelected = true; // Set the flag to true after marking
    }
  });
