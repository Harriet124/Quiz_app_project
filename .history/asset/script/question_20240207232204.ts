      ...
      if (filteredData.length > 0) {
        ...
        function displayCurrentQuestion() {
          ...
          submitButton.addEventListener("click", () => {
            // Check if a submission has already occurred
            ...
            // Set submissionOccurred to true after submission
...
let submissionOccurred = false; // Add this boolean flag

function markOption(optionsList, optionElement) {
  const markedOption = optionsList.querySelector(".marked");

  // Check if a submission has occurred
  if (submissionOccurred) {
    return;
  }
  ...
  if (markedOption || optionElement.classList.contains("marked")) {
    ...
    const alphabetSpan = markedOption.querySelector(".letter");
  ...
  const alphabetSpan = optionElement.querySelector(".letter");
