...
optionsList.forEach((option) => {
  option.addEventListener("mouseenter", () => {
    const letter = option.querySelector(".letter") as HTMLElement;
    letter.style.backgroundColor = "#8f0eec40";
    letter.style.color = "#A729F5";
  });

  option.addEventListener("mouseleave", () => {
    const letter = option.querySelector(".letter") as HTMLElement;
    letter.style.backgroundColor = ""; // Reset to the default background color
    letter.style.color = ""; // Reset to the default text color
  });
});
